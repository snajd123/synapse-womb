...
fn test_genome_random_in_range() {
    for _ in 0..50 {
        ...
        assert!(g.cortisol_strength >= 0.1 && g.cortisol_strength <= 0.8,
            "cortisol_strength {} out of range", g.cortisol_strength);
    }
}
...
fn test_genome_crossover_combines_parents() {
    ...
    parent_a.cortisol_strength = 0.1;
    ...
    for _ in 0..100 {
        ...
        if (child.cortisol_strength - 0.1).abs() < 0.001 { got_a = true; }
...
fn test_tune_tiny_run() {
    ...
    assert!(best_genome.cortisol_strength >= 0.1 && best_genome.cortisol_strength <= 0.8);
}
